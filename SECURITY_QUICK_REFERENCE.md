...
# 2. Update deploy.sh CORS_ORIGINS
# Edit deploy.sh and set:
# --set-env-vars "ENVIRONMENT=production,CORS_ORIGINS=https://yourdomain.com"
...
# Production
curl https://odace-pipeline-588398598428.europe-west1.run.app/health
```
...
gcloud run logs read odace-pipeline --region europe-west1 \
  --filter="textPayload=~'Invalid admin secret|Invalid API key'"

# Rate limit violations
gcloud run logs read odace-pipeline --region europe-west1 \
  --filter="textPayload=~'Rate limit exceeded'"

# Startup security validations
gcloud run logs read odace-pipeline --region europe-west1 \
  --filter="textPayload=~'ADMIN_SECRET|CORS'"
```
...
export SERVICE_URL="http://localhost:8080"  # or your Cloud Run URL
